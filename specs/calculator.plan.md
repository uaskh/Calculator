...
| D-12 | Keyboard focus | Input focused on load; "type anywhere": printable keys, Backspace and Escape routed to the input from the body and from buttons, Enter/Space on buttons stay native, Enter on the body commits (user, 2026-09-20; spec decision 38) | Supersedes A-29; new hook `useTypeAnywhere`, component and Playwright tests |
| D-11 | Desktop layout | From 48 rem a centred panel ≤ 64 rem: display across the top, keypad left, History right (column reserved while empty), sizes scale with the viewport, keys ≥ 44 px (user, 2026-09-20; spec decision 37) | Layout CSS rewritten; Playwright layout assertions at 1280 px |
...
| A-29 | Superseded by D-12: Escape clears from anywhere on the page; keypad and history buttons still act on Enter/Space | Recorded for history |
