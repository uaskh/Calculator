...
- Inexact operations — `/`, `^` with a non-integer exponent, `sqrt` — are computed with at
  ...
  `exp(f·ln x)` evaluated in binary fixed point on `math/big` integers resolving 80, 112 or
  172 decimal places (a tier chosen from the base's integer digits: 8, 40 or 100 digits +
  32 places + 40 guard places, with an exact `2^k` split so no error is amplified), because
  the
...
| `INVALID_NUMBER`         | `invalid number '1.2.3' at character N` (the whole number token is quoted; `N` is where it starts, decision 39)     |
...
| `^-?(0                   | [1-9][0-9]_)(\.[0-9]_[1-9])?$` (the pattern goes into the OpenAPI schema).                                          |
...
- **Look** (decision 40): a physical calculator: the calculator is a "device body" panel
  on a soft gradient page; the display is a dark LCD in both schemes (display text ≥ 7:1)
  holding the label, the edit line, the result and "Evaluated as"; keys are rounded with a
  lit top, a 3 px edge and a press-down state, coloured by group (digits neutral,
  operators blue-tinted, `C`/`⌫` warm, `=` accent); history is a paper-tape strip. System
  fonts only; every new colour is a token with light and dark values and its contrast is
  asserted by the token unit test; key edges (the visible control border) are ≥ 3:1.
